...
/// re-write the file as we search it, skipping over the line we want to remove 
    ...
    function get_all() {
        $all = $this->db->get('m_routes')->result();
        $routes = array();
    ...
    function get($id) {
        return $this->db->get_where('mc_routes', array('id_route' => $id))->row();
    }

    // save or update a route and return the id
    function save($route) {
        if (!empty($route['id_route'])) {
            $this->db->where('id_route', $route['id_route']);
            $this->db->update('mc_routes', $route);

            return $route['id_route'];
        } else {            
            $this->db->insert('mc_routes', $route);
    ...
    function check_slug($slug, $id = false) {
        ...
        return (bool) $this->db->count_all_results('mc_routes');
    }
